...
function cmdd(ctx: any) {
  ctx.message.text = ctx.message.text.replace(new RegExp("^\\" + config.startSymbol + "[a-zA-Z0-9]{2,20}@" + ctx.botInfo.username, 'i'),
...
let stage = new Scenes.Stage<Scenes.SceneContext>([cScene, pyScene, jsScene, cppScene, jvScene, goScene, tsScene, addScene], { ttl: config.ttl });
